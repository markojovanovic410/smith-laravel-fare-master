                        ...
                        <div class="cart-totals-fields">
                           ...
                           <div class="widget-body green">
                              <div class="price-wrap text-center">
                                 ...
                                 */
                                 ?>
                                 <a id="placeOrder" type="button" class="btn btn-primary bg-white primary-color placeOrder">Place Order <i class="fa fa-arrow-right" aria-hidden="true"></i></a>
                                 <p id="msg"></p>
                              </div>
                           </div>
                        </div>
         ...
         success: (data, textStatus, jqXHR) => {
            if((data.responseData).length != 0) { 
              ...
              if(cart_list.store_type=='FOOD'){
               ...
               total =parseInt(cart_list.payable.toFixed(2));
               wallet_balance=parseInt(cart_list.user_wallet_balance.toFixed(2));
...
$('body').on('click','.placeOrder',function(){
      ...
      var inside_total = total

      if($('#wallet').is(':checked')) {
         var wallet = 1;
         if(wallet_balance > inside_total){
            inside_total=0;
         }else{
            inside_total=inside_total-wallet_balance;
         }
      ...
      if(payment_mode=="CARD"){ 
        if(card_id==''){
      ...
      }else if(payment_mode=="PAYSTACK"){

          if(inside_total == 0){
            $('#msg').html("Total Amount Should be greater than zero. Please Change Payment mode");
            //alertMessage("Error","Total Amount Should be greater than zero", "error");
            return false;
           }else{
              $('#msg').html("");
           }
          ...
          var handler = PaystackPop.setup({ 
            ...
            amount: inside_total*100, //amount the customer is supposed to pay
            metadata: {
                custom_fields: [
                    {
