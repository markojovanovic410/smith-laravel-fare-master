...
<script>
   ...
   $(document).ready(function() {
      ...
      if(userSettings.id == 0) {
         ...
         $('.header-submenu').append(`<a class="dropdown-item" href="{{ url('user/login') }}"><i class="material-icons">account_circle</i> Login</a>`);
      ...
      $('#service').on('change', function(){
         ...
         var userSettings = getUserDetails();
         $.ajax({
               url: getBaseUrl() + "/user/service_city_price/"+city_service+"?city_id="+userSettings.card_id,
               type: "GET",
               headers: {
                     Authorization: "Bearer " + getToken("user")
                  },
